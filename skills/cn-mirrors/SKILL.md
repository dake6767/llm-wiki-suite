...
| **winget (Windows)** | install "succeeds" per manifest but download fails | winget's own CDN is fine in CN, but packages whose installer fetches from GitHub can still fail. Prefer a project's own CDN or a directly verifiable non-GitHub release; My LLM Wiki's official FFmpeg ships inside its SHA-256-verified distribution pack. | — |
