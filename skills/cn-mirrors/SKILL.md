...
| **winget (Windows)** | install "succeeds" per manifest but download fails | winget's own CDN is fine in CN, but **packages whose installer fetches from GitHub** (e.g. FFmpeg builds) still fail — fall back to the tool's non-GitHub distribution (ffmpeg: gyan.dev builds, in this suite via the structured `cn` recipe `scripts/fetch_ffmpeg.py`) or pip where one exists | — |
