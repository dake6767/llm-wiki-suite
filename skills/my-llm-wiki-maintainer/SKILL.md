...
- **Tag vocabulary**: `wiki_ops.py tags <root>` prints the wiki's established tags with counts, derived from the pages themselves — no stored vocabulary file to maintain or drift. **Ingest must read this before writing any tag** (see `references/ingest-update.md` step 7). It is what makes schema.md's "reusable across ≥2 pages" rule satisfiable: without it an agent coins tags blind and every session fragments the facet with fresh near-synonyms. The default view is bounded by `--limit`, so it costs the same (~340 tokens) on a 900-page wiki as on a 20-page one and stays inside the O(top-k) budget. `--audit` adds singletons, duplicate pairs and untagged pages — a cleanup view, never for ingest.
