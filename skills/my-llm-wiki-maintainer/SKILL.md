...
- **Tag vocabulary**: `wiki_ops.py tags <root>` prints the wiki's live tag vocabulary — established tags with counts, singletons, and possible duplicate pairs — derived from the pages themselves. **Ingest must read this before writing any tag** (see `references/ingest-update.md` step 7). It is what makes schema.md's "reusable across ≥2 pages" rule satisfiable: without it an agent coins tags blind and every session fragments the facet with fresh near-synonyms. The output is deliberately tiny and does not count against the O(top-k) retrieval budget.
